...
### 更新が反映されない時の対処（Service Worker）
1. ブラウザのサイト設定から「ストレージ/キャッシュ」を削除  
2. 画面を再読み込み（必要ならタブを閉じて再度開く）  
3. それでも反映されない場合は一度オフライン機能を無効化してから再読み込み  
   - Chrome: DevTools → Application → Service Workers → Unregister  
   - Safari: 開発メニュー → Service Workers で該当サイトを削除  
...
### multi（複数選択）
- `options` は必須
- `answer` は選択肢キーの配列（採点は **集合一致**：選んだキー集合と正答集合が完全一致した場合のみ正解）
```json
{
  "id": "Q002",
  "type": "multi",
